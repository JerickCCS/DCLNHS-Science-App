            ...
            <q-page class="q-pa-md">
                ...
                <div class="instructions-card q-mb-lg">
                    <div class="instructions-header">
                        ...
                        <div class="instruction-text">
                            ...
                            paragraph about trade winds and the ITCZ.
                        </div>
                    </div>
                </div>
                ...
                <div class="paragraph-container q-mt-lg">
                    ...
                    <div class="safety-paragraph">
                        <p>
                            Trade winds are air currents near the
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'equator')">
                                {{ blanks.equator || '________' }}
                            </span>
                            that are deflected by the
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'coriolis')">
                                {{ blanks.coriolis || '________' }}
                            </span>
                            Effect. The ITCZ is where these winds from the Northern and Southern Hemispheres
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'converge')">
                                {{ blanks.converge || '________' }}
                            </span>.
                            Its position changes
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'seasonally')">
                                {{ blanks.seasonally || '________' }}
                            </span>,
                            moving north during the Northern Hemisphere's summer.
                        </p>

                        <p>
                            In the Philippines, the ITCZ brings
                            <span class="blank" @dragover.prevent @dragenter.prevent @drop="dropWord($event, 'rains')">
                                {{ blanks.rains || '________' }}
                            </span>
                            and
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'thunderstorms')">
                                {{ blanks.thunderstorms || '________' }}
                            </span>.
                            Over land, the ITCZ moves more
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'poleward')">
                                {{ blanks.poleward || '________' }}
                            </span>
                            than over water. Sailors call it the
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'doldrums')">
                                {{ blanks.doldrums || '________' }}
                            </span>
                            because of little surface wind.
                        </p>
                    </div>
                </div>
...
const config = {
    quizName: 'Mini Test 11-4',
    type: 'mini_test',  // Added type to match first example
    totalItems: 8,      // 8 fill-in-the-blank items
    chapter: 11,        // Changed from unit to chapter to match first example
    lesson: 2           // Updated to match the quiz number
}
...
const initialKeywords = [
    ...
    { text: 'Coriolis', correctFor: 'coriolis', used: false },
    { text: 'converge', correctFor: 'converge', used: false },
    { text: 'seasonally', correctFor: 'seasonally', used: false },
    { text: 'rains', correctFor: 'rains', used: false },
    { text: 'thunderstorms', correctFor: 'thunderstorms', used: false },
    { text: 'poleward', correctFor: 'poleward', used: false },
    { text: 'doldrums', correctFor: 'doldrums', used: false }
]
...
const briefFeedback = computed(() => {
    const tier = getScoreTier(score.value, config.totalItems)
    const feedback = {
        perfect: 'Perfect! You understand trade winds and the ITCZ.',
        excellent: 'Excellent understanding of atmospheric circulation!',
        good: 'Good understanding of weather concepts, keep practicing!',
        tryAgain: 'Review trade winds and the ITCZ. Try again to improve your score!'
    }
