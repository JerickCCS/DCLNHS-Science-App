                ...
                <div class="instructions-card q-mb-lg">
                    <div class="instructions-header">
                        ...
                        <div class="instruction-text">
                            ...
                            paragraphs about Earth's motions and seasons.
                        </div>
                    </div>
                </div>
                ...
                <div class="paragraph-container q-mt-lg">
                    ...
                    <div class="safety-paragraph">
                        <p class="paragraph-section">
                            <strong>1. Earth's Motions:</strong><br>
                            The Earth has two main motions: rotation and revolution.
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'rotation')">
                                {{ blanks.rotation || '________' }}
                            </span>
                            is Earth's spinning on its axis, taking 24 hours and causing day and night.
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'revolution')">
                                {{ blanks.revolution || '________' }}
                            </span>
                            is Earth's orbit around the Sun, taking approximately 365.25 days and responsible for the
                            changing seasons.
                        </p>

                        <p class="paragraph-section">
                            <strong>2. Equatorial Warmth:</strong><br>
                            The equator receives more direct and concentrated
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'insolation')">
                                {{ blanks.insolation || '________' }}
                            </span>
                            year-round due to the Earth's
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'curvature')">
                                {{ blanks.curvature || '________' }}
                            </span>.
                            At polar regions, sunlight spreads over a larger area and travels through more atmosphere,
                            reducing its intensity. This difference in solar energy distribution creates a temperature
                            gradient.
                        </p>

                        <p class="paragraph-section">
                            <strong>3. The Equinox:</strong><br>
                            During an equinox, which occurs in March and September, the Sun shines directly on the
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'equator')">
                                {{ blanks.equator || '________' }}
                            </span>,
                            resulting in nearly equal
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'dayAndNight')">
                                {{ blanks.dayAndNight || '________' }}
                            </span>
                            worldwide. Both hemispheres receive similar amounts of solar radiation during these periods.
                        </p>

                        <p class="paragraph-section">
                            <strong>4. Seasonal Variations:</strong><br>
                            Areas between the tropics and polar circles experience four distinct seasons due to
                            significant changes in the
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'sunAngle')">
                                {{ blanks.sunAngle || '________' }}
                            </span>
                            and
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'dayLength')">
                                {{ blanks.dayLength || '________' }}
                            </span>
                            throughout the year. The equator experiences only wet and dry seasons because the Sun's
                            <span class="blank" @dragover.prevent @dragenter.prevent
                                @drop="dropWord($event, 'sunlight')">
                                {{ blanks.sunlight || '________' }}
                            </span>
                            remains relatively constant, with minimal variation in solar intensity.
                        </p>
                    </div>
                </div>
...
const config = {
    quizName: 'Mini Test 12-1',
    type: 'mini_test',  // Added type to match first example
    totalItems: 9,      // Updated to 9 fill-in-the-blank items
    chapter: 12,        // Changed from unit to chapter to match first example
    lesson: 1           // Updated to match the quiz number
}
...
const initialKeywords = [
    { text: 'Rotation', correctFor: 'rotation', used: false },
    { text: 'Revolution', correctFor: 'revolution', used: false },
    { text: 'insolation', correctFor: 'insolation', used: false },
    { text: 'curvature', correctFor: 'curvature', used: false },
    { text: 'equator', correctFor: 'equator', used: false },
    { text: 'day and night', correctFor: 'dayAndNight', used: false },
    { text: 'sun angle', correctFor: 'sunAngle', used: false },
    { text: 'day length', correctFor: 'dayLength', used: false },
    { text: 'sunlight', correctFor: 'sunlight', used: false }
]
...
const briefFeedback = computed(() => {
    const tier = getScoreTier(score.value, config.totalItems)
    const feedback = {
        perfect: 'Perfect! You understand Earth\'s motions and seasons.',
        excellent: 'Excellent understanding of Earth science concepts!',
        good: 'Good understanding of Earth science, keep practicing!',
        tryAgain: 'Review Earth\'s motions and seasons. Try again to improve your score!'
    }
